...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const description = `Daftar paket internet IDPlay ${speed} Mbps dengan pembayaran ${period.toLowerCase()} seharga Rp ${priceInfo.price}/bulan. Nikmati koneksi super cepat dan stabil.`;
    ...
    <div className="bg-gray-50 min-h-screen">
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <Link href="/cek-jangkauan" className="bg-[#ec6210] text-white font-bold py-2 px-6 rounded-lg hover:bg-opacity-90 transition-all duration-300">
            Cek Jangkauan
          </Link>
        </div>
      </header>
      ...
      <main className="container mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col gap-4">
            {plan.badge && <span className="text-sm font-bold text-[#ec6210]">{plan.badge}</span>}
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mt-4">
              ...
              {priceInfo.total && (
                <div className="text-right mt-2 text-sm text-green-600 font-semibold">
                  Total {periodMap[period]}: Rp {priceInfo.total} (Anda hemat Rp {priceInfo.savings})
                </div>
              )}
            </div>

            <Link href="/cek-jangkauan" className="mt-6 w-full text-center bg-[#ec6210] text-white font-bold py-4 px-8 rounded-lg text-xl hover:bg-opacity-90 transition-all duration-300 transform hover:scale-105">
              Langganan Sekarang
            </Link>
          </div>
        </div>
        ...
        <div className="mt-20 bg-white p-8 rounded-lg shadow-lg">
          <h2 className="text-3xl font-extrabold text-gray-800 mb-4">{content.title}</h2>
